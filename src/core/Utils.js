...
const Utils = {
     ...
     */
    padBytesRight: function padBytesRight(arr, numBytes, padByte = 0) {
     ...
     * @returns {string}
     */
    printable: function printable(str, preserveWs = false, onlyAscii = false) {
        if (onlyAscii) {
     ...
     * @returns {string}
     */
    escapeWhitespace: function escapeWhitespace(str) {
     ...
     */
    parseEscapedChars: function parseEscapedChars(str) {
        return str.replace(
     ...
     * Utils.expandAlphRange("a-d0\\-3")
     */
    expandAlphRange: function expandAlphRange(alphStr) {
        const alphArr = [];
     ...
     */
    convertToByteArray: function convertToByteArray(str, type) {
     ...
     */
    convertToByteString: function convertToByteString(str, type) {
        switch (type.toLowerCase()) {
            case "binary":
     ...
     */
    byteArrayToInt: function byteArrayToInt(byteArray, byteorder) {
        let value = 0;
        if (byteorder === "big") {
     ...
     */
    intToByteArray: function intToByteArray(value, length, byteorder) {
     ...
     */
    strToArrayBuffer: function strToArrayBuffer(str) {
        log.debug(`Converting string[${str && str.length}] to array buffer`);
     ...
     */
    strToUtf8ArrayBuffer: function strToUtf8ArrayBuffer(str) {
        log.debug(
            `Converting string[${str && str.length}] to UTF8 array buffer`
        );
        if (!str) return new ArrayBuffer();
    ...
    strToByteArray: function strToByteArray(str) {
        log.debug(`Converting string[${str && str.length}] to byte array`);
        if (!str) return [];
     ...
     */
    strToUtf8ByteArray: function strToUtf8ByteArray(str) {
        log.debug(`Converting string[${str && str.length}] to UTF8 byte array`);
        if (!str) return [];
     ...
     */
    strToCharcode: function strToCharcode(str) {
        log.debug(`Converting string[${str && str.length}] to charcode`);
        if (!str) return [];
        const charcode = [];
     ...
     */
    byteArrayToUtf8: function byteArrayToUtf8(byteArray) {
        log.debug(
            `Converting byte array[${byteArray && byteArray.length}] to UTF8`
        );
     ...
     */
    byteArrayToChars: function byteArrayToChars(byteArray) {
        log.debug(
            `Converting byte array[${byteArray && byteArray.length}] to chars`
        );
        if (!byteArray || !byteArray.length) return "";
        let str = "";
     ...
     */
    arrayBufferToStr: function arrayBufferToStr(arrayBuffer, utf8 = true) {
        log.debug(
     ...
     * @returns {number}
     */
    calculateShannonEntropy: function calculateShannonEntropy(data) {
     ...
     * @returns {string}
     */
    encodeURIFragment: function encodeURIFragment(str) {
        const LEGAL_CHARS = {
            "%2D": "-",
            "%2E": ".",
            "%5F": "_",
     ...
     * @returns {string}
     */
    generatePrettyRecipe: function generatePrettyRecipe(
        recipeConfig,
        newline = false
    ) {
        let prettyConfig = "",
     ...
     * @returns {Object[]}
     */
    parseRecipeConfig: function parseRecipeConfig(recipe) {
        recipe = recipe.trim();
     ...
     * @returns {html}
     */
    displayFilesAsHTML: async function displayFilesAsHTML(files) {
     ...
     * Utils.parseURIParams("#a=abc&b=123")
     */
    parseURIParams: function parseURIParams(paramStr) {
        if (paramStr === "") return {};
     ...
     * @returns {string}
     */
    charRep: function charRep(token) {
        return {
            Space: " ",
            Percent: "%",
            Comma: ",",
     ...
     * @returns {RegExp}
     */
    regexRep: function regexRep(token) {
        return {
            Space: /\s+/g,
            Percent: /%/g,
            Comma: /,/g,
     ...
     */
    chunked: function* chunked(iterable, chunksize) {

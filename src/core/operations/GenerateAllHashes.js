...
class GenerateAllHashes extends Operation {
    ...
    async run(input, args) {
        ...
        if (length === "All") {
            output += "\nChecksums:\n";
            for (const checksum of this.checksums) {
                digest = await this.executeAlgo(
                    checksum.algo,
                    checksum.inputType,
                    checksum.params || []
     ...
     * @returns {string}
     */
    async executeAlgo(algo, inputType, params = []) {
        let digest = null;
        switch (inputType) {
            case "arrayBuffer":
                digest = await algo.run(this.inputArrayBuffer, params);
                break;
            case "str":
                digest = await algo.run(this.inputStr, params);
                break;
            case "byteArray":
                digest = await algo.run(this.inputByteArray, params);
                break;
            default:
